...
RegisterNetEvent('psr-weathersync:server:setWeather', function(weather)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (success) then TriggerClientEvent('PSRCore:Notify', source, Lang:t('weather.updated'), 'success')
            else TriggerClientEvent('PSRCore:Notify', source, Lang:t('weather.invalid'), 'error')
            end
        end
    end
end)
...
RegisterNetEvent('psr-weathersync:server:setTime', function(hour, minute)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (success) then TriggerClientEvent('PSRCore:Notify', src, 9, Lang:t('time.change', {value = hour, value2 = minute or "00"}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
            else TriggerClientEvent('PSRCore:Notify', source, Lang:t('time.invalid'), 'error')
            end
        end
    end
end)
...
RegisterNetEvent('psr-weathersync:server:toggleBlackout', function(state)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then TriggerClientEvent('PSRCore:Notify', source, Lang:t('blackout.enabled'), 'success')
            else TriggerClientEvent('PSRCore:Notify', source, Lang:t('blackout.disabled'), 'success')
            end
        end
    end
end)
...
RegisterNetEvent('psr-weathersync:server:toggleFreezeTime', function(state)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then TriggerClientEvent('PSRCore:Notify', source, Lang:t('time.now_frozen'), 'success')
            else TriggerClientEvent('PSRCore:Notify', source, Lang:t('time.now_unfrozen'), 'inform')
            end
        end
    end
end)
...
RegisterNetEvent('psr-weathersync:server:toggleDynamicWeather', function(state)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then TriggerClientEvent('PSRCore:Notify', source, Lang:t('weather.now_unfrozen'), 'success')
            else TriggerClientEvent('PSRCore:Notify', source, Lang:t('weather.now_frozen'), 'success')
            end
        end
    end
end)
...
RegisterCommand('freezetime', function(source, args, rawCommand)  
    if isAllowedToChange(source) then
        ...
        if source > 0 then
            if (newstate) then return TriggerClientEvent('PSRCore:Notify', source, Lang:t('time.frozenc'), 'success') end
            return TriggerClientEvent('PSRCore:Notify', source, Lang:t('time.unfrozenc'), 'success')
        end
        if (newstate) then return print(Lang:t('time.now_frozen')) end
        return print(Lang:t('time.now_unfrozen'))
    end
    TriggerClientEvent('PSRCore:Notify', source, Lang:t('error.not_allowed'), 'error')
end)
...
RegisterCommand('freezeweather', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        ...
        if source > 0 then
            if (newstate) then return TriggerClientEvent('PSRCore:Notify', source, Lang:t('dynamic_weather.enabled'), 'success') end
            return TriggerClientEvent('PSRCore:Notify', source, Lang:t('dynamic_weather.disabled'), 'success')
        end
...
RegisterCommand('weather', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        if args[1] == nil then
            if source > 0 then return TriggerClientEvent('PSRCore:Notify', source, Lang:t('weather.invalid_syntaxc'), 'success') end
            return print(Lang:t('weather.invalid_syntax'))
        end
        local success = setWeather(args[1])
        if source > 0 then
            if (success) then return TriggerClientEvent('PSRCore:Notify', source, Lang:t('weather.willchangeto'), 'success') end
            return TriggerClientEvent('PSRCore:Notify', source, Lang:t('weather.invalidc'), 'success')
        end
...
RegisterCommand('blackout', function(source, args, rawCommand)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then return TriggerClientEvent('PSRCore:Notify', source, Lang:t('blackout.enabledc'), 'sucess') end
            return TriggerClientEvent('PSRCore:Notify', source, Lang:t('blackout.disabledc'), 'sucess')
        end
...
RegisterCommand('morning', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        setTime(9, 0)
        if source > 0 then return TriggerClientEvent('PSRCore:Notify', source, Lang:t('time.morning'), 'success')end
    else
        TriggerClientEvent('PSRCore:Notify', source, Lang:t('error.not_allowed'), 'error')
    end
end)
...
RegisterCommand('noon', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        setTime(12, 0)
        if source > 0 then return TriggerClientEvent('PSRCore:Notify', source, Lang:t('time.noon'), 'success') end
    else
        TriggerClientEvent('PSRCore:Notify', source, Lang:t('error.not_allowed'), 'error')
    end
end)
...
RegisterCommand('evening', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        setTime(18, 0)
        if source > 0 then return TriggerClientEvent('PSRCore:Notify', source, Lang:t('time.evening'), 'success') end
    else
        TriggerClientEvent('PSRCore:Notify', source, Lang:t('error.not_allowed'), 'error')
    end
end)
...
RegisterCommand('night', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        setTime(23, 0)
        if source > 0 then return TriggerClientEvent('PSRCore:Notify', source, Lang:t('time.night'), 'success') end
    else
        TriggerClientEvent('PSRCore:Notify', source, Lang:t('error.not_allowed'), 'error')
    end
end)
...
RegisterCommand('time', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        if args[1] == nil then
            if source > 0 then return TriggerClientEvent('PSRCore:Notify', source, Lang:t('time.invalidc'), 'error') end
            return print(Lang:t('time.invalid'))
        end
        local success = setTime(args[1], args[2])
        if source > 0 then
            if (success) then return TriggerClientEvent('PSRCore:Notify', source, 9, Lang:t('time.changec', {value = args[1]..':'..(args[2] or "00")}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE') end
            return TriggerClientEvent('PSRCore:Notify', source, Lang:t('time.invalidc'), 'error')
        end
        if (success) then return print(Lang:t('time.change', {value = args[1], value2 = args[2] or "00"})) end
        return print(Lang:t('time.invalid'))
    else
        TriggerClientEvent('PSRCore:Notify', source, Lang:t('time.access'), 'error')
    end
end)
